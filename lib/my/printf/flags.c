...
void printf_my_putptr(void *pointer, char flag, int *buff)
{
    ...
    if (flag == 'o')
        my_putnbr_base(pointer, "01234567");
    if (flag == 'x')
        my_putnbr_base(pointer, "0123456789abcdef");
    if (flag == 'X')
        my_putnbr_base(pointer, "0123456789ABCDEF");
    if (flag == 'u')
        my_putnbr_base(pointer, "0123456789");
    if (flag == 'b')
        my_putnbr_base(pointer, "01");
    if (flag == 'S')
        my_megaputstr(pointer, (buff[6] >= 2) ? buff[4] : my_strlen(pointer));
    if (flag == 's')
        my_putnstr(pointer, (buff[6] >= 2) ? buff[4] : my_strlen(pointer));
}
